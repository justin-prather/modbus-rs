//! Go cgo surface for the async Modbus TCP gateway.
//!
//! # Lifecycle
//!
//! 1. Call [`mbus_go_tcp_gateway_new`] with the upstream bind address.
//! 2. Register downstream channels with [`mbus_go_tcp_gateway_add_downstream`].
//! 3. Add routing rules with [`mbus_go_tcp_gateway_add_unit_route`] and/or
//!    [`mbus_go_tcp_gateway_add_range_route`].
//! 4. Call [`mbus_go_tcp_gateway_start`] to begin serving on a background thread.
//! 5. Call [`mbus_go_tcp_gateway_stop`] to shut down.
//! 6. Call [`mbus_go_tcp_gateway_free`] to release the handle.
...
use crate::go::status::MbusGoStatus;

use super::router::GoRouter;

/// Opaque gateway handle.
pub struct MbusGoTcpGateway {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mbus_go_tcp_gateway_new(
    host: *const c_char,
    port: u16,
) -> *mut MbusGoTcpGateway {
    if host.is_null() {
        return ptr::null_mut();
    }
    ...
    Box::into_raw(Box::new(MbusGoTcpGateway {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mbus_go_tcp_gateway_free(handle: *mut MbusGoTcpGateway) {
    if !handle.is_null() {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mbus_go_tcp_gateway_add_downstream(
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mbus_go_tcp_gateway_add_unit_route(
    handle: *mut MbusGoTcpGateway,
    unit_id: u8,
    channel: u32,
) -> MbusGoStatus {
...
/// `handle` must be a valid gateway pointer.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mbus_go_tcp_gateway_add_range_route(
    handle: *mut MbusGoTcpGateway,
    unit_min: u8,
    unit_max: u8,
    channel: u32,
...
/// `handle` must be a valid gateway pointer.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mbus_go_tcp_gateway_stop(handle: *mut MbusGoTcpGateway) {
