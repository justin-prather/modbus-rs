...
fn gateway_forwards_request_and_returns_response() {
    ...
    assert_eq!(
        upstream_sent.len(),
        1,
        "gateway should have sent one response upstream"
    );
    ...
    assert_eq!(
        upstream_response.transaction_id(),
        0x0001,
...
fn gateway_poll_returns_ok_when_no_upstream_data() {
    ...
    assert!(
        result.is_ok(),
        "poll with no upstream data should return Ok(())"
    );
...
fn gateway_sends_exception_on_routing_miss() {
    ...
    assert_eq!(
        upstream_sent.len(),
        1,
        "gateway should have sent an exception upstream"
    );
...
fn gateway_propagates_downstream_timeout() {
    ...
    assert_eq!(
        gw.event_handler().timeout_count,
        1,
        "timeout callback should fire once"
    );
}
...
fn gateway_rewrites_txn_id_for_downstream() {
    ...
    assert_eq!(
        ds_msg.transaction_id(),
        0,
        "downstream should use internal txn_id 0"
    );
    ...
    assert_eq!(
        us_msg.transaction_id(),
        0xABCD,
        "upstream response should use original txn_id"
    );
}
...
fn gateway_applies_unit_id_rewrite_for_downstream() {
    ...
    let ds_msg = decompile_adu_frame(&ds_sent[0], TransportType::StdTcp).unwrap();
    assert_eq!(
        ds_msg.unit_id_or_slave_addr().get(),
        105,
        "downstream should use rewritten unit ID"
    );
    ...
    let us_msg = decompile_adu_frame(&us_sent[0], TransportType::StdTcp).unwrap();
    assert_eq!(
        us_msg.unit_id_or_slave_addr().get(),
        5,
        "upstream response should use original unit ID"
    );
}
...
fn gateway_event_callbacks_fire_on_success() {
    ...
    let response_adu = build_tcp_request(0x0000, 3, FunctionCode::ReadCoils, &[0x01, 0x0F]);
    ...
    let mut gw: GatewayServices<MockTransport, MockTransport, _, _, 1> = GatewayServices::new(
        upstream,
        router,
        Recorder {
            forwards: 0,
