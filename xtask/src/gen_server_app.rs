...
pub fn run(opts: &GenServerAppOptions) -> Result<(), String> {
    ...
    let rust_text_and_out = opts.out_dir.as_ref().map(|out_dir| {
        (mbus_codegen::render_rust_dispatcher(&config), out_dir.join("generated_server.rs"))
    });
    let header_text_and_path = opts.emit_c_header.as_ref().map(|hp| {
        (mbus_codegen::render_c_header(&config), hp.clone())
    });
    ...
    if let Some((rust_text, rust_out)) = &rust_text_and_out {
        let out_dir = rust_out.parent().expect("generated_server.rs has no parent dir");
    ...
    println!("Generated server app artifacts from {}", opts.config_path.display());
    Ok(())
}
