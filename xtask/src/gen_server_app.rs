...
pub fn run(opts: &GenServerAppOptions) -> Result<(), String> {
    ...
    if let (Some(target), Some(output_root)) = (&opts.target, output_root) {
        ...
        // 6e. Copy modbus_rs.h into include/
        // This header is generated by cbindgen at build time; it lives in
        // target/mbus-ffi/include/modbus_rs.h.  We copy it if present.
        let server_header_src = Path::new("target/mbus-ffi/include/modbus_rs.h");
        if server_header_src.exists() {
            let dest = include_dir.join("modbus_rs.h");
            fs::copy(server_header_src, &dest)
                .map_err(|e| format!("failed to copy modbus_rs.h: {e}",))?;
            println!("  copied modbus_rs.h -> {}", dest.display());
            // Also copy to alt include dir if present
            if let Some(ref alt) = alt_include_dir {
                let alt_dest = alt.join("modbus_rs.h");
