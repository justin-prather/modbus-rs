...
impl FullApp {
    fn new() -> Self {
        ...
        Self {
            ...
            fifo_queue: [0xAAAA, 0xBBBB, 0xCCCC, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000],
            fifo_count: 3,
            file_store,
...
impl ModbusAppHandler for FullApp {
    ...
    fn read_device_identification_request(
        ...
        for &(id, val) in DEVICE_OBJECTS.iter().filter(|&&(id, _)| id >= start_object_id) {
            let needed = 2 + val.len();
...
fn main() {
    ...
    #[cfg(feature = "coils")]
    {
        ...
        let n = app
            ...
            .expect("FC01");
        println!("FC01 coils[0..16] packed = {:#010b} {:#010b}  ({n} bytes)", out[0], out[1]);
    }
    ...
    #[cfg(feature = "discrete-inputs")]
    {
        ...
        let n = app
            ...
            .expect("FC02");
        println!("FC02 discrete_inputs[0..8] packed = {:#010b}  ({n} bytes)", out[0]);
    }
    ...
    #[cfg(feature = "fifo")]
    {
        ...
        let vals: Vec<u16> = (0..fifo_count as usize)
            ...
            .collect();
        println!("FC18 fifo[{FIFO_POINTER_ADDR:#06X}]: count={fifo_count}, values={vals:04X?}  ({n} bytes)");
    }
    ...
    #[cfg(feature = "file-record")]
    {
        ...
        let r: Vec<u16> = (0..2)
            .map(|i| u16::from_be_bytes([out[i * 2], out[i * 2 + 1]]))
            .collect();
        println!("FC14/FC15 file1 record[10..12]: wrote [0xABCD, 0xEF01], read {r:04X?}  ({n} bytes)");
    }
    ...
    {
        let status = app
            .read_exception_status_request(70, uid)
            .expect("FC07");
