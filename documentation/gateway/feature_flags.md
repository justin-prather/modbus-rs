...
```toml
[dependencies]
mbus-gateway = { version = "0.9.0", default-features = false, features = ["traffic"] }
```
...
```toml
mbus-gateway = { version = "0.9.0", features = ["ws-server", "serial-rtu"] }
```
...
```toml
[dependencies]
modbus-rs = { version = "0.9.0", features = ["gateway"] }
```
